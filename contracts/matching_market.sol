...
contract MatchingMarket is DSAuth, DSMath, DSNote {
    ...
    mapping(string => mapping(string => uint)) public _best;  //id of the highest offer for a token pair
    mapping(string => mapping(string => uint)) public _span;  //number of offers stored for token pair in sorted orderbook
    mapping(string => uint) public _dust;                      //minimum sell amount for a token to avoid dust offers
    ...
    //    of tokens received.
    function setMinSell(
        string memory pay_gem,     //token to assign minimum sell amount to
    ...
    function getMinSell(
        string memory pay_gem      //token for which minimum sell amount is queried
    )
        public
        view
        returns (uint)
    ...
    function getBestOffer(string memory sell_gem, string memory buy_gem) public view returns(uint) {
    ...
    function getOfferCount(string memory sell_gem, string memory buy_gem) public view returns(uint) {
    ...
    function sellAllAmount(string memory pay_gem, uint pay_amt, string memory buy_gem, uint min_fill_amount)
        public
    ...
    function buyAllAmount(string memory buy_gem, uint buy_amt, string memory pay_gem, uint max_fill_amount)
        public
        returns (uint fill_amt)
    {
        require(!locked, "Reentrancy attempt");
    ...
    function getBuyAmount(string memory buy_gem, string memory pay_gem, uint pay_amt) public view returns (uint fill_amt) {
    ...
    function getPayAmount(string memory pay_gem, string memory buy_gem, uint buy_amt) public view returns (uint fill_amt) {
        uint256 offerId = getBestOffer(buy_gem, pay_gem);           //Get best offer for the token pair
        ...
        returns (uint)
    {
        ...
        string memory buy_gem = offers[id].buy_gem;
        string memory pay_gem = offers[id].pay_gem;
    ...
    function _matcho(
        ...
        string memory t_buy_gem,   //taker buy which token
        ...
        internal
    {
        ...
        pos = pos == 0 || keccak256(bytes(offers[pos].pay_gem)) != keccak256(bytes(pay_gem)) || keccak256(bytes(offers[pos].buy_gem)) != keccak256(bytes(buy_gem)) || !isOfferSorted(pos)
        ?
            _find(id)
        :
            _findpos(id, pos);
